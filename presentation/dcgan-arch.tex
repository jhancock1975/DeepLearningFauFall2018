...
\node[medium] (noise) at (0,0) {Noise Vector};
...
\node[circle,fill=green!20] (adder) at (4.5,4){+};
...
\draw[->] (discriminator) -- (dCostDataset);
\draw[->] (discriminator) -- (gCostGenerated);
\draw[->] (discriminator) -- (dCostGenerated);

\draw[->] (dCostGenerated) -- (8,2) |- (adder);
\draw[->] (dCostDataset) -- (adder);
